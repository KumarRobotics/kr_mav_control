...
public:
  ...
  enum Status { INIT, MOTORS_OFF, IDLE, ELAND, ESTOP, FLYING };
  ...
  bool setDesVelInWorldFrame(
      float x, float y, float z, float yaw, bool use_position_feedback = false);
  bool setDesVelInBodyFrame(
      float x, float y, float z, float yaw, bool use_position_feedback = false);
  ...
  bool compound_lissajous(float x_amp[2],
                          float y_amp[2],
                          float z_amp[2],
                          float yaw_amp[2],
...
private:
  typedef actionlib::SimpleActionClient<kr_tracker_msgs::LineTrackerAction>
      ClientType;
  typedef actionlib::SimpleActionClient<kr_tracker_msgs::CircleTrackerAction>
      CircleClientType;
  typedef actionlib::SimpleActionClient<kr_tracker_msgs::LissajousTrackerAction>
      LissajousClientType;
  typedef actionlib::SimpleActionClient<kr_tracker_msgs::LissajousAdderAction>
      CompoundLissajousClientType;

  ros::NodeHandle nh_;
  ros::NodeHandle priv_nh_;

  void tracker_done_callback(
      const actionlib::SimpleClientGoalState& state,
      const kr_tracker_msgs::LineTrackerResultConstPtr& result);
  void circle_tracker_done_callback(
      const actionlib::SimpleClientGoalState& state,
      const kr_tracker_msgs::CircleTrackerResultConstPtr& result);
  void lissajous_tracker_done_callback(
      const actionlib::SimpleClientGoalState& state,
      const kr_tracker_msgs::LissajousTrackerResultConstPtr& result);
  void lissajous_adder_done_callback(
      const actionlib::SimpleClientGoalState& state,
  ...
  // Publishers
  ros::Publisher pub_motors_, pub_estop_, pub_goal_yaw_, pub_goal_velocity_,
      pub_so3_command_, pub_trpy_command_, pub_position_command_, pub_status_,
      pub_pwm_command_;

  // Subscribers
  ros::Subscriber odom_sub_, imu_sub_, output_data_sub_, heartbeat_sub_,
      tracker_status_sub_;
