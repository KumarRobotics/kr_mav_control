...
// Services
ros::Subscriber sub_motors_, sub_takeoff_, sub_goTo_,
    sub_setDesVelInWorldFrame_, sub_setDesVelInBodyFrame_, sub_hover_,
...
int main(int argc, char** argv) {
  ...
  // Services
  sub_motors_ =
      nh.subscribe("motors", 10, &MAV_Subscribers::motors_cb, &mav_subs);
  sub_takeoff_ =
      nh.subscribe("takeoff", 10, &MAV_Subscribers::takeoff_cb, &mav_subs);
  sub_goTo_ = nh.subscribe("goTo", 10, &MAV_Subscribers::goTo_cb, &mav_subs);
  sub_setDesVelInWorldFrame_ =
      nh.subscribe("setDesVelInWorldFrame",
                   10,
                   &MAV_Subscribers::setDesVelInWorldFrame_cb,
                   &mav_subs);
  sub_setDesVelInBodyFrame_ =
      nh.subscribe("setDesVelInBodyFrame",
                   10,
                   &MAV_Subscribers::setDesVelInBodyFrame_cb,
                   &mav_subs);
  sub_hover_ = nh.subscribe("hover", 10, &MAV_Subscribers::hover_cb, &mav_subs);
  sub_ehover_ =
      nh.subscribe("ehover", 10, &MAV_Subscribers::ehover_cb, &mav_subs);
