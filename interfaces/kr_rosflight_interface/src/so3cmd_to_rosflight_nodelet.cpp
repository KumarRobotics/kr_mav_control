...
void SO3CmdToRosflight::so3_cmd_callback(
  ...
  tf::Quaternion q_des_tf = tf::Quaternion(msg->orientation.x,
                                           msg->orientation.y,
                                           msg->orientation.z,
                                           msg->orientation.w);
  ...
  {
    ROS_WARN_THROTTLE(
        1, "Psi %2.2f > 1.0, orientation error is too large!", Psi);
  }
...
void SO3CmdToRosflight::onInit(void) {
  ...
  so3_cmd_sub_ = priv_nh.subscribe("so3_cmd",
                                   10,
                                   &SO3CmdToRosflight::so3_cmd_callback,
                                   this,
                                   ros::TransportHints().tcpNoDelay());

  odom_sub_ = priv_nh.subscribe("odom",
                                10,
                                &SO3CmdToRosflight::odom_callback,
                                this,
                                ros::TransportHints().tcpNoDelay());

  imu_sub_ = priv_nh.subscribe("imu",
                               10,
                               &SO3CmdToRosflight::imu_callback,
                               this,
                               ros::TransportHints().tcpNoDelay());
}
