...
void SO3CmdToMavros::so3_cmd_callback(
  ...
  const Eigen::Quaterniond q_des_transformed =
      Eigen::Quaterniond(tf_imu_odom_yaw.w(),
                         tf_imu_odom_yaw.x(),
                         tf_imu_odom_yaw.y(),
                         tf_imu_odom_yaw.z()) *
      q_des;
...
void SO3CmdToMavros::onInit(void) {
  ...
  if (priv_nh.getParam("kf", kf_))
    ROS_INFO("Using kf=%g so that prop speed = sqrt(f / num_props / kf) to "
             "scale force to speed.",
             kf_);
  else
    ROS_ERROR("Must set kf param for thrust scaling. Motor speed = sqrt(thrust "
              "/ num_props / kf)");
  ...
  if (priv_nh.getParam("lin_cof_a", lin_cof_a_) &&
    ...
    ROS_INFO("Using %g*x + %g to scale prop speed to att_throttle.",
             lin_cof_a_,
             lin_int_b_);
  else
  ...
  so3_cmd_sub_ = priv_nh.subscribe("so3_cmd",
                                   10,
                                   &SO3CmdToMavros::so3_cmd_callback,
                                   this,
                                   ros::TransportHints().tcpNoDelay());

  odom_sub_ = priv_nh.subscribe("odom",
                                10,
                                &SO3CmdToMavros::odom_callback,
                                this,
                                ros::TransportHints().tcpNoDelay());

  imu_sub_ = priv_nh.subscribe("imu",
                               10,
                               &SO3CmdToMavros::imu_callback,
                               this,
                               ros::TransportHints().tcpNoDelay());
}
