...
SnavSampler::SnavSampler(ros::NodeHandle& nh, ros::NodeHandle& pnh)
  ...
  if (rpm_rate > 1e-3) {
    ...
    rpm_timer_ = nh.createTimer(
        ros::Duration(1.0 / rpm_rate), &SnavSampler::rpmTimerCallback, this);
  }
  if (status_rate > 1e-3) {
    ROS_INFO("Publish status (battery, spektrum_joy, on_ground, props_state) "
             "at %4.2fHz",
             status_rate);
    ...
    status_timer_ = nh.createTimer(ros::Duration(1.0 / status_rate),
                                   &SnavSampler::statusTimerCallback,
                                   this);
  }
  if (attitude_rate > 1e-3) {
    ROS_INFO("Publish attitude estimates at %4.2fHz", attitude_rate);
    attitude_estimate_pub_ =
        nh.advertise<geometry_msgs::QuaternionStamped>("attitude_estimate", 2);
    attitude_estimate_timer_ =
        nh.createTimer(ros::Duration(1.0 / attitude_rate),
                       &SnavSampler::attitudeTimerCallback,
                       this);
  }
}
...
void SnavSampler::attitudeTimerCallback(const ros::TimerEvent& event) {
  if (sn_update_data() != 0)
    ...
    tf2::Matrix3x3 R(sn_struct_->attitude_estimate.rotation_matrix[0],
...
void SnavSampler::statusTimerCallback(const ros::TimerEvent& event) {
  if (sn_update_data() != 0)
    ...
    if ((SnPropsState)sn_struct_->general_status.props_state ==
        SN_PROPS_STATE_NOT_SPINNING) {
      props_state_msg.data = "NOT_SPINNING";
    } else if ((SnPropsState)sn_struct_->general_status.props_state ==
               SN_PROPS_STATE_STARTING) {
