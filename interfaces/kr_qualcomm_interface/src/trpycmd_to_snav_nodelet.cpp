...
void TRPYCmdToSnav::odom_callback(const nav_msgs::Odometry::ConstPtr& odom) {
  ...
  if (trpy_cmd_set_ &&
    ...
    const auto last_trpy_cmd_ptr =
        boost::make_shared<kr_mav_msgs::TRPYCommand>(last_trpy_cmd_);
...
void TRPYCmdToSnav::imu_callback(const sensor_msgs::Imu::ConstPtr& pose) {
  ...
  imu_q_ = Eigen::Quaterniond(pose->orientation.w,
                              pose->orientation.x,
                              pose->orientation.y,
                              pose->orientation.z);
...
void TRPYCmdToSnav::motors_on() {
  ...
  case SN_PROPS_STATE_NOT_SPINNING: {
    sn_send_thrust_att_ang_vel_command(
...
void TRPYCmdToSnav::motors_off() {
  ...
  } while (snav_cached_data_struct_->general_status.props_state ==
           SN_PROPS_STATE_SPINNING);

  // check the propellers status
  if (snav_cached_data_struct_->general_status.props_state ==
      SN_PROPS_STATE_SPINNING) {
...
void TRPYCmdToSnav::trpy_cmd_to_qc_interface(
    const kr_mav_msgs::TRPYCommand::ConstPtr& msg) {
  const tf::Quaternion q_rot =
      tf::createQuaternionFromRPY(msg->roll, msg->pitch, msg->yaw);
  Eigen::Quaterniond q_des;
  tf::quaternionTFToEigen(q_rot, q_des);

  const Eigen::Vector3d ang_vel(msg->angular_velocity.x,
  ...
  int r = sn_send_thrust_att_ang_vel_command(throttle,
                                             q_des.w(),
                                             q_des.x(),
                                             q_des.y(),
                                             q_des.z(),
...
void TRPYCmdToSnav::onInit(void) {
  ...
  if (sn_get_flight_data_ptr(sizeof(SnavCachedData),
                             &snav_cached_data_struct_) != 0) {
    ROS_ERROR("\nFailed to get flight data pointer!\n");
    return;
  }

  trpy_cmd_sub_ = priv_nh.subscribe("trpy_cmd",
                                    10,
                                    &TRPYCmdToSnav::trpy_cmd_callback,
                                    this,
                                    ros::TransportHints().tcpNoDelay());

  odom_sub_ = priv_nh.subscribe("odom",
                                10,
                                &TRPYCmdToSnav::odom_callback,
                                this,
                                ros::TransportHints().tcpNoDelay());

  imu_sub_ = priv_nh.subscribe("imu",
                               10,
                               &TRPYCmdToSnav::imu_callback,
                               this,
                               ros::TransportHints().tcpNoDelay());
}
