...
//  linear.z = thrust   [0 to 60,000]               (motors stiction around
//  2000) angular.z = yawrate [-200 to 200 degrees/second] (note this is not
//  yaw!)
...
void SO3CmdToCrazyflie::odom_callback(
    const nav_msgs::Odometry::ConstPtr& odom) {
  if (!odom_set_)
    odom_set_ = true;

  q_odom_ = Eigen::Quaterniond(odom->pose.pose.orientation.w,
...
void SO3CmdToCrazyflie::so3_cmd_callback(
  ...
  if (msg->aux.enable_motors) {
    // If the crazyflie motors are timed out, we need to send a zero message in
    // order to get them to start
    if (motor_status_ < 3) {
  ...
  /*
  // If the crazyflie motors are timed out, we need to send a zero message in
  order to get them to start if((ros::Time::now() - last_so3_cmd_time_).toSec()
  >= so3_cmd_timeout_){ crazy_cmd_vel_pub_.publish(crazy_vel_cmd);
    // Keep in mind that this publishes two messages in a row, this may cause
  bandwith problems
  }
  */
  ...
  const Eigen::Quaterniond q_des(msg->orientation.w,
                                 msg->orientation.x,
                                 msg->orientation.y,
                                 msg->orientation.z);
  ...
  Eigen::Matrix3d R_des_new =
      R_des * Eigen::AngleAxisd(yaw_cur - yaw_des, Eigen::Vector3d::UnitZ());
  ...
  double thrust_f = f_des(0) * R_cur(0, 2) + f_des(1) * R_cur(1, 2) +
                    f_des(2) * R_cur(2, 2); // Force in Newtons
  ...
  float yaw_rate_des =
      ((-msg->kR[2] * e_yaw) - msg->angular_velocity.z) * (180 / M_PI);
  ...
  // ROS_INFO("commanded thrust is %2.2f", CLAMP(thrust_pwm, thrust_pwm_min_,
  // thrust_pwm_max_));
  ...
  //}
  // else {
  //  ROS_INFO_STREAM("Commands too quick, time since is: " << (ros::Time::now()
  //  - last_so3_cmd_time_).toSec());
  //}
}
...
void SO3CmdToCrazyflie::onInit(void) {
  ...
  // get thrust scaling parameters
  if (priv_nh.getParam("c1", c1_) && priv_nh.getParam("c2", c2_) &&
      priv_nh.getParam("c3", c3_))
