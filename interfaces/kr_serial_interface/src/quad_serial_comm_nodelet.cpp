...
void QuadSerialComm::serial_read_callback(const unsigned char* data,
                                          size_t count) {
  process_serial_data(
      data,
      count,
      boost::bind(&QuadSerialComm::output_data_callback, this, _1));
}
...
void QuadSerialComm::onInit(void) {
  ...
  serial_sub_ = n.subscribe("to_robot",
                            10,
                            &QuadSerialComm::serial_callback,
                            this,
                            ros::TransportHints().tcpNoDelay());

  sd_.SetReadCallback(
      boost::bind(&QuadSerialComm::serial_read_callback, this, _1, _2));
  sd_.Start();
}
