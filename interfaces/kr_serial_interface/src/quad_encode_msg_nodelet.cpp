...
void QuadEncodeMsg::so3_cmd_callback(
...
void QuadEncodeMsg::trpy_cmd_callback(
...
void QuadEncodeMsg::pwm_cmd_callback(
    const kr_mav_msgs::PWMCommand::ConstPtr& msg) {
  kr_mav_msgs::Serial::Ptr serial_msg(new kr_mav_msgs::Serial);
  serial_msg->header.seq = msg->header.seq;
  serial_msg->channel = channel_;
...
void QuadEncodeMsg::onInit(void) {
  ...
  so3_cmd_sub_ = priv_nh.subscribe("so3_cmd",
                                   10,
                                   &QuadEncodeMsg::so3_cmd_callback,
                                   this,
                                   ros::TransportHints().tcpNoDelay());
  trpy_cmd_sub_ = priv_nh.subscribe("trpy_cmd",
                                    10,
                                    &QuadEncodeMsg::trpy_cmd_callback,
                                    this,
                                    ros::TransportHints().tcpNoDelay());
  pwm_cmd_sub_ = priv_nh.subscribe("pwm_cmd",
                                   10,
                                   &QuadEncodeMsg::pwm_cmd_callback,
                                   this,
                                   ros::TransportHints().tcpNoDelay());
}
