...
SO3Control::SO3Control()
    : mass_(0.5), g_(9.81), max_pos_int_(0.5), max_pos_int_b_(0.5),
      current_orientation_(Eigen::Quaternionf::Identity()),
      cos_max_tilt_angle_(-1.0) {}

void SO3Control::setMass(const float mass) { mass_ = mass; }

void SO3Control::setGravity(const float g) { g_ = g; }
...
void SO3Control::setCurrentOrientation(
...
void SO3Control::calculateControl(const Eigen::Vector3f& des_pos,
  ...
  // ROS_DEBUG_THROTTLE(2, "Integrated world disturbance compensation [N]: {x:
  // %2.2f, y: %2.2f, z: %2.2f}", pos_int_(0), pos_int_(1), pos_int_(2));
  ...
  // ROS_DEBUG_THROTTLE(2, "Integrated body disturbance compensation [N]: {x:
  // %2.2f, y: %2.2f, z: %2.2f}", pos_int_b_(0), pos_int_b_(1), pos_int_b_(2));

  const Eigen::Vector3f acc_grav = g_ * Eigen::Vector3f::UnitZ();
  const Eigen::Vector3f acc_control =
      kx.asDiagonal() * e_pos + kv.asDiagonal() * e_vel + pos_int_ + des_acc;
  Eigen::Vector3f acc_total = acc_control + acc_grav;

  // Check and limit tilt angle
  float lambda = 1.0f;
  if (Eigen::Vector3f::UnitZ().dot(acc_total.normalized()) <
      cos_max_tilt_angle_) {
    const float x = acc_control.x(), y = acc_control.y(), z = acc_control.z();
    const float cot_max_tilt_angle =
        cos_max_tilt_angle_ /
        std::sqrt(1 - cos_max_tilt_angle_ * cos_max_tilt_angle_);
  ...
  const Eigen::Vector3f force_dot =
      mass_ * lambda *
      (kx.asDiagonal() * e_vel +
       des_jerk); // Ignoring kv*e_acc and ki*e_pos terms
  const Eigen::Vector3f b3c_dot =
      b3c.cross(force_dot / force_.norm()).cross(b3c);
  const Eigen::Vector3f b2d_dot(
      -std::cos(des_yaw) * des_yaw_dot, -std::sin(des_yaw) * des_yaw_dot, 0);
  const Eigen::Vector3f b1c_dot = b1c.cross(
      ((b2d_dot.cross(b3c) + b2d.cross(b3c_dot)) / (b2d.cross(b3c)).norm())
          .cross(b1c));
  ...
  angular_velocity_ =
      Eigen::Vector3f(omega_hat(2, 1), omega_hat(0, 2), omega_hat(1, 0));
}

const Eigen::Vector3f& SO3Control::getComputedForce() { return force_; }
