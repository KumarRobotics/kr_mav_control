...
public:
  SO3TRPYControlNodelet()
      : odom_set_(false), position_cmd_updated_(false),
        position_cmd_init_(false), des_yaw_(0), des_yaw_dot_(0), yaw_int_(0),
        enable_motors_(false), use_external_yaw_(false), g_(9.81) {
...
private:
  ...
  Eigen::Vector3f des_pos_, des_vel_, des_acc_, des_jrk_, config_kx_,
      config_kv_, config_ki_, config_kib_, kx_, kv_;
  ...
  typedef dynamic_reconfigure::Server<kr_mav_controllers::SO3Config>
      ReconfigureServer;
...
void SO3TRPYControlNodelet::publishCommand() {
  ...
  controller_.calculateControl(des_pos_,
                               des_vel_,
                               des_acc_,
                               des_jrk_,
                               des_yaw_,
  ...
  if (Psi < 1.0f) // Position control stability guaranteed only when Psi < 1
    thrust = force(0) * R_cur(0, 2) + force(1) * R_cur(1, 2) +
             force(2) * R_cur(2, 2);
...
void SO3TRPYControlNodelet::position_cmd_callback(
  ...
  des_acc_ = Eigen::Vector3f(
      cmd->acceleration.x, cmd->acceleration.y, cmd->acceleration.z);
  des_jrk_ = Eigen::Vector3f(cmd->jerk.x, cmd->jerk.y, cmd->jerk.z);

  // Check use_msg_gains_flag to decide whether to use gains from the msg or
  // config
  kx_[0] = (cmd->use_msg_gains_flags & cmd->USE_MSG_GAINS_POSITION_X)
               ? cmd->kx[0]
               : config_kx_[0];
  kx_[1] = (cmd->use_msg_gains_flags & cmd->USE_MSG_GAINS_POSITION_Y)
               ? cmd->kx[1]
               : config_kx_[1];
  kx_[2] = (cmd->use_msg_gains_flags & cmd->USE_MSG_GAINS_POSITION_Z)
               ? cmd->kx[2]
               : config_kx_[2];
  kv_[0] = (cmd->use_msg_gains_flags & cmd->USE_MSG_GAINS_VELOCITY_X)
               ? cmd->kv[0]
               : config_kv_[0];
  kv_[1] = (cmd->use_msg_gains_flags & cmd->USE_MSG_GAINS_VELOCITY_Y)
               ? cmd->kv[1]
               : config_kv_[1];
  kv_[2] = (cmd->use_msg_gains_flags & cmd->USE_MSG_GAINS_VELOCITY_Z)
               ? cmd->kv[2]
               : config_kv_[2];
...
void SO3TRPYControlNodelet::odom_callback(
  ...
  current_orientation_ = Eigen::Quaternionf(odom->pose.pose.orientation.w,
...
void SO3TRPYControlNodelet::enable_motors_callback(
...
void SO3TRPYControlNodelet::corrections_callback(
    const kr_mav_msgs::Corrections::ConstPtr& msg) {
  corrections_[0] = msg->kf_correction;
  corrections_[1] = msg->angle_corrections[0];
  corrections_[2] = msg->angle_corrections[1];
}

void SO3TRPYControlNodelet::cfg_callback(kr_mav_controllers::SO3Config& config,
                                         uint32_t level) {
  if (level == 0) {
  ...
  if (level & (1 << 0)) {
    ...
    NODELET_INFO("Position Gains set to kp: {%2.3g, %2.3g, %2.3g}, kd: {%2.3g, "
                 "%2.3g, %2.3g}",
                 config_kx_[0],
                 config_kx_[1],
                 config_kx_[2],
  ...
  if (level & (1 << 1)) {
    ...
    NODELET_INFO("Integral Gains set to ki: {%2.2g, %2.2g, %2.2g}, kib: "
                 "{%2.2g, %2.2g, %2.2g}",
                 config_ki_[0],
                 config_ki_[1],
                 config_ki_[2],
  ...
  if (level & (1 << 2)) {
    ...
    NODELET_INFO("Attitude Gains set to kp: {%2.2g, %2.2g, %2.2g}, kd: {%2.2g, "
                 "%2.2g, %2.2g}",
                 kR_[0],
                 kR_[1],
                 kR_[2],
  ...
  if (level & (1 << 4)) {
    ...
    NODELET_INFO("Maxes set to Integral: %2.2g, Integral Body: %2.2g, Tilt "
                 "Angle (rad): %2.2g",
                 config.max_pos_int,
  ...
  NODELET_WARN_STREAM_COND(
      level != std::numeric_limits<uint32_t>::max() && (level >= (1 << 5)),
      "kr_mav_controllers dynamic reconfigure called, but with unknown level: "
          << level);
}
...
void SO3TRPYControlNodelet::onInit() {
  ...
  reconfigure_server_->setCallback(
      boost::bind(&SO3TRPYControlNodelet::cfg_callback, this, _1, _2));

  trpy_command_pub_ = n.advertise<kr_mav_msgs::TRPYCommand>("trpy_cmd", 10);

  odom_sub_ = n.subscribe("odom",
                          10,
                          &SO3TRPYControlNodelet::odom_callback,
                          this,
                          ros::TransportHints().tcpNoDelay());
  position_cmd_sub_ = n.subscribe("position_cmd",
                                  10,
  ...
  enable_motors_sub_ =
      n.subscribe("motors",
                  2,
                  &SO3TRPYControlNodelet::enable_motors_callback,
                  this,
                  ros::TransportHints().tcpNoDelay());
  corrections_sub_ = n.subscribe("corrections",
                                 10,
                                 &SO3TRPYControlNodelet::corrections_callback,
                                 this,
                                 ros::TransportHints().tcpNoDelay());
}
