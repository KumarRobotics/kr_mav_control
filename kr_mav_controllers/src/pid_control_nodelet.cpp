...
public:
  PIDControlNodelet()
      : position_cmd_updated_(false), position_cmd_init_(false), des_yaw_(0),
        current_yaw_(0), enable_motors_(false), use_external_yaw_(false) {}
...
void PIDControlNodelet::publishTRPYCommand(void) {
  ...
  controller_.calculateControl(
      des_pos_, des_vel_, des_acc_, des_yaw_, kx_, kv_, ki, ki_yaw);
...
void PIDControlNodelet::position_cmd_callback(
...
void PIDControlNodelet::odom_callback(
...
void PIDControlNodelet::enable_motors_callback(
...
void PIDControlNodelet::onInit(void) {
  ...
  odom_sub_ = n.subscribe("odom",
                          10,
                          &PIDControlNodelet::odom_callback,
                          this,
                          ros::TransportHints().tcpNoDelay());
  position_cmd_sub_ = n.subscribe("position_cmd",
                                  10,
                                  &PIDControlNodelet::position_cmd_callback,
                                  this,
                                  ros::TransportHints().tcpNoDelay());

  enable_motors_sub_ = n.subscribe("motors",
                                   2,
