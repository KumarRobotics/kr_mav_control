...
void PIDControl::setMass(const float mass) { mass_ = mass; }

void PIDControl::setGravity(const float g) { g_ = g; }
...
void PIDControl::setYaw(const float current_yaw) { current_yaw_ = current_yaw; }
...
void PIDControl::calculateControl(const Eigen::Vector3f& des_pos,
  ...
  Eigen::Vector3f force_des = kx.asDiagonal() * e_pos + pos_int_ +
                              kv.asDiagonal() * e_vel + mass_ * des_acc +
                              mass_ * g_ * Eigen::Vector3f(0, 0, 1);

  float roll_des =
      (force_des(0) * sin(current_yaw_) - force_des(1) * cos(current_yaw_)) /
      force_des(2);
  float pitch_des =
      (force_des(0) * cos(current_yaw_) + force_des(1) * sin(current_yaw_)) /
      force_des(2);
...
const Eigen::Vector4f& PIDControl::getControls(void) { return trpy_; }
