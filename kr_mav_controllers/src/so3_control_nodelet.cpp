...
public:
  SO3ControlNodelet()
      : position_cmd_updated_(false), position_cmd_init_(false), des_yaw_(0),
        des_yaw_dot_(0), current_yaw_(0), enable_motors_(false),
        use_external_yaw_(false), have_odom_(false), g_(9.81),
...
void SO3ControlNodelet::cfg_callback(kr_mav_controllers::SO3Config& config,
                                     uint32_t level) {
  if (level == 0) {
...
void SO3ControlNodelet::onInit(void) {
  ...
  reconfigure_server_ =
      boost::make_shared<ReconfigureServer>(config_mutex_, priv_nh);
  reconfigure_server_->updateConfig(config);
  reconfigure_server_->setCallback(
      boost::bind(&SO3ControlNodelet::cfg_callback, this, _1, _2));

  so3_command_pub_ = priv_nh.advertise<kr_mav_msgs::SO3Command>("so3_cmd", 10);
  command_viz_pub_ =
      priv_nh.advertise<geometry_msgs::PoseStamped>("cmd_viz", 10);

  odom_sub_ = priv_nh.subscribe("odom",
                                10,
                                &SO3ControlNodelet::odom_callback,
                                this,
                                ros::TransportHints().tcpNoDelay());
  position_cmd_sub_ =
      priv_nh.subscribe("position_cmd",
                        10,
                        &SO3ControlNodelet::position_cmd_callback,
                        this,
                        ros::TransportHints().tcpNoDelay());
  enable_motors_sub_ =
      priv_nh.subscribe("motors",
                        2,
                        &SO3ControlNodelet::enable_motors_callback,
                        this,
                        ros::TransportHints().tcpNoDelay());
  corrections_sub_ = priv_nh.subscribe("corrections",
                                       10,
