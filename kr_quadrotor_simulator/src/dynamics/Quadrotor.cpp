...
void Quadrotor::setInput(double u1, double u2, double u3, double u4) {
  ...
  input_(3) = u4;
  if (u1 != 0 || u2 != 0 || u3 != 0 ||
      u4 != 0) // Limit to min/max RPM if any of the RPMs are non-zero
  {
    for (int i = 0; i < 4; i++) {
...
const Quadrotor::State& Quadrotor::getState() const { return state_; }
...
double Quadrotor::getMass() const { return mass_; }
void Quadrotor::setMass(double mass) { mass_ = mass; }

double Quadrotor::getDragCoefficient() const { return drag_coefficient_; }

void Quadrotor::setDragCoefficient(double drag_coefficient) {
  drag_coefficient_ = drag_coefficient;
}

double Quadrotor::getGravity() const { return g_; }
void Quadrotor::setGravity(double g) { g_ = g; }

const Eigen::Matrix3d& Quadrotor::getInertia() const { return J_; }
...
double Quadrotor::getArmLength() const { return arm_length_; }
...
double Quadrotor::getPropRadius() const { return prop_radius_; }
...
double Quadrotor::getPropellerThrustCoefficient() const { return kf_; }
...
double Quadrotor::getPropellerMomentCoefficient() const { return km_; }
...
double Quadrotor::getMotorTimeConstant() const { return motor_time_constant_; }
...
double Quadrotor::getMaxRPM() const { return max_rpm_; }
...
double Quadrotor::getMinRPM() const { return min_rpm_; }
