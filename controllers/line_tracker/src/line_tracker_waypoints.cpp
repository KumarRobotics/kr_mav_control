...
class LineTrackerWaypoints : public controllers_manager::Controller
{
 public:
  LineTrackerWaypoints(void);
 ...
 private:
  ...
  std::stack<quadrotor_msgs::FlatOutputs::ConstPtr> goal_list_;
...
LineTrackerWaypoints::LineTrackerWaypoints(void) :
    pos_set_(false),
    goal_set_(false),
    goal_reached_(true),
    active_(false)
...
void LineTrackerWaypoints::Initialize(const ros::NodeHandle &nh)
{
  ...
  ros::NodeHandle priv_nh(nh, "line_tracker_waypoints");
  ...
  sub_goal_ = priv_nh.subscribe("goal", 10, &LineTrackerWaypoints::goal_callback, this,
                                ros::TransportHints().tcpNoDelay());
  sub_path_ = priv_nh.subscribe("waypoints", 10, &LineTrackerWaypoints::path_callback, this,
                                ros::TransportHints().tcpNoDelay());
 
  srv_param_ = priv_nh.advertiseService("set_des_vel_acc", &LineTrackerWaypoints::set_des_vel_acc, this);
}

bool LineTrackerWaypoints::Activate(void)
{
...
void LineTrackerWaypoints::Deactivate(void)
{
  goal_set_ = false;
  active_ = false;
}

const quadrotor_msgs::PositionCommand::Ptr LineTrackerWaypoints::update(const nav_msgs::Odometry::ConstPtr &msg)
{
  ...
  if(goal_reached_ && !goal_list_.empty())
    send_next_goal();
...
void LineTrackerWaypoints::send_next_goal()
{
  goal_callback(goal_list_.top());
  goal_list_.pop();
}

void LineTrackerWaypoints::path_callback(const nav_msgs::Path::ConstPtr& msg)
{
  while(!goal_list_.empty())
    goal_list_.pop();
  //for(size_t i = 0; i < msg->poses.size(); i++)
  for(int i = msg->poses.size() - 1; i >= 0; i--)
  {
    quadrotor_msgs::FlatOutputs::Ptr g(new quadrotor_msgs::FlatOutputs);
    g->x = msg->poses[i].pose.position.x;
    g->y = msg->poses[i].pose.position.y;
    g->z = msg->poses[i].pose.position.z;

    g->yaw = tf::getYaw(msg->poses[i].pose.orientation);
    goal_list_.push(g);
  }
  goal_set_ = true;
  goal_reached_ = false;
...
void LineTrackerWaypoints::goal_callback(const quadrotor_msgs::FlatOutputs::ConstPtr &msg)
{
  goal_pos_(0) = msg->x;
  goal_pos_(1) = msg->y;
  goal_pos_(2) = msg->z;
...
bool LineTrackerWaypoints::set_des_vel_acc(line_tracker::DesVelAcc::Request &req,
                                     line_tracker::DesVelAcc::Response &res)
{
...
PLUGINLIB_EXPORT_CLASS(LineTrackerWaypoints, controllers_manager::Controller);
