...
#include <kr_so3_control/SO3Config.h>
...
class SO3TRPYControlNodelet : public nodelet::Nodelet
{
 ...
 private:
  ...
  typedef dynamic_reconfigure::Server<kr_so3_control::SO3Config> ReconfigureServer;
...
void SO3TRPYControlNodelet::cfg_callback(kr_so3_control::SO3Config &config, uint32_t level)
{
  if (level == 0)
  {
  ...
  NODELET_WARN_STREAM_COND(level != std::numeric_limits<uint32_t>::max() && (level >= (1 << 5)),
                           "kr_so3_control dynamic reconfigure called, but with unknown level: " << level);
}
...
void SO3TRPYControlNodelet::onInit()
{
  ...
  kr_so3_control::SO3Config config;
