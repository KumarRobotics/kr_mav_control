...
class TrajectoryGenerator
{
 public:
   ...
   */
  TrajectoryGenerator(unsigned int continuous_derivative_order,
                      unsigned int minimize_derivative);

  void setInitialConditions(const Vec3f &position,
                            const vec_Vec3f &derivatives);
  void addWaypoint(const Vec3f &position); // Waypoint is X, Y, Z
  void clearWaypoints(void);
  std::vector<float> computeTimesTrapezoidSpeed(float vel_des,
                                                float acc_des) const;
  std::vector<float> computeTimesConstantSpeed(float avg_speed) const;
  bool calculate(const std::vector<float> &waypoint_times_);
  bool getCommand(const float time, Vec3f &pos, Vec3f &vel, Vec3f &acc,
                  Vec3f &jrk) const;

  void calcMaxPerSegment(std::vector<float> &max_vel,
                         std::vector<float> &max_acc,
                         std::vector<float> &max_jrk) const;

  void optimizeWaypointTimes(const float max_vel, const float max_acc,
                             const float max_jrk);
 ...
 private:
  ...
  std::vector<Eigen::MatrixX3f, Eigen::aligned_allocator<Eigen::MatrixX3f>>
      coefficients_;
