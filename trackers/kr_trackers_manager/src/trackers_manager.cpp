...
TrackersManager::~TrackersManager(void) {
  for (std::map<std::string, kr_trackers_manager::Tracker*>::iterator it =
           tracker_map_.begin();
       it != tracker_map_.end();
       it++) {
    delete it->second;
    ...
    try {
      ...
      NODELET_ERROR_STREAM("Could not unload library for the tracker "
                           << it->first << ": " << e.what());
    }
#endif
  }
...
void TrackersManager::onInit(void) {
  ...
  for (int i = 0; i < tracker_list.size(); i++) {
    ...
    const std::string tracker_name =
        static_cast<const std::string>(tracker_list[i]);
    try {
#if ROS_VERSION_MINIMUM(1, 8, 0)
      kr_trackers_manager::Tracker* c =
          tracker_loader_.createUnmanagedInstance(tracker_name);
#else
      kr_trackers_manager::Tracker* c =
          tracker_loader_.createClassInstance(tracker_name);
#endif
      c->Initialize(priv_nh);
      tracker_map_.insert(std::make_pair(tracker_name, c));
    } catch (pluginlib::LibraryLoadException& e) {
      NODELET_ERROR_STREAM("Could not load library for the tracker "
                           << tracker_name << ": " << e.what());
    } catch (pluginlib::CreateClassException& e) {
      NODELET_ERROR_STREAM("Could not create an instance of the tracker "
                           << tracker_name << ": " << e.what());
    }
  }
  ...
  sub_odom_ = priv_nh.subscribe("odom",
                                10,
                                &TrackersManager::odom_callback,
                                this,
                                ros::TransportHints().tcpNoDelay());

  srv_tracker_ = priv_nh.advertiseService(
      "transition", &TrackersManager::transition_callback, this);
}
...
void TrackersManager::odom_callback(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  for (it = tracker_map_.begin(); it != tracker_map_.end(); it++) {
    if (it->second == active_tracker_) {
      ...
      kr_tracker_msgs::TrackerStatus::Ptr status_msg(
          new kr_tracker_msgs::TrackerStatus);
...
bool TrackersManager::transition_callback(
    kr_tracker_msgs::Transition::Request& req,
    kr_tracker_msgs::Transition::Response& res) {
  const std::map<std::string, kr_trackers_manager::Tracker*>::iterator it =
      tracker_map_.find(req.tracker);
  if (it == tracker_map_.end()) {
    res.success = false;
    res.message = std::string("Cannot find tracker ") + req.tracker +
                  std::string(", cannot transition");
  ...
  if (active_tracker_ == it->second) {
    res.success = true;
    res.message =
        std::string("Tracker ") + req.tracker + std::string(" already active");
  ...
  if (!it->second->Activate(cmd_)) {
    res.success = false;
    res.message = std::string("Failed to activate tracker ") + req.tracker +
                  std::string(", cannot transition");
    NODELET_WARN_STREAM(res.message);
    return true;
  }
