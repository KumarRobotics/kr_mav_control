...
private:
  ...
  typedef actionlib::SimpleActionServer<kr_tracker_msgs::LissajousAdderAction>
      ServerType;
...
void LissajousAdder::Initialize(const ros::NodeHandle& nh) {
  ...
  tracker_server_ = std::shared_ptr<ServerType>(
      new ServerType(priv_nh, "LissajousAdder", false));
  tracker_server_->registerGoalCallback(
      boost::bind(&LissajousAdder::goal_callback, this));
  tracker_server_->registerPreemptCallback(
      boost::bind(&LissajousAdder::preempt_callback, this));
  tracker_server_->start();
}

bool LissajousAdder::Activate(
  ...
  if (generator_1_.goalIsSet() && generator_2_.goalIsSet()) {
    if (!tracker_server_->isActive()) {
      ROS_WARN("LissajousAdder::Activate: goal_set is true but action server "
               "has no active goal - not activating.");
      return false;
    }
...
kr_mav_msgs::PositionCommand::ConstPtr
LissajousAdder::update(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  if (!traj_start_set_) {
    ...
    position_last_ =
        Eigen::Vector3d(ICs_.pos()(0), ICs_.pos()(1), ICs_.pos()(2));
  ...
  if (cmd1 == NULL && cmd2 == NULL) {
    ...
    if (!generator_1_.status() || !generator_2_.status()) {
      ...
      feedback.time_to_completion =
          std::max(generator_1_.timeRemaining(), generator_2_.timeRemaining());
      ...
      result.duration =
          std::max(generator_1_.timeElapsed(), generator_2_.timeElapsed());
...
void LissajousAdder::goal_callback(void) {
  ...
  kr_tracker_msgs::LissajousAdderGoal::ConstPtr msg =
      tracker_server_->acceptNewGoal();
