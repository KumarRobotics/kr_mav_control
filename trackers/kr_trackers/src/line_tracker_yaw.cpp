...
void LineTrackerYaw::Initialize(const ros::NodeHandle& nh) {
  ...
  sub_goal_ = priv_nh.subscribe("goal",
                                10,
                                &LineTrackerYaw::goal_callback,
                                this,
                                ros::TransportHints().tcpNoDelay());
}

bool LineTrackerYaw::Activate(
    const kr_tracker_msgs::PositionCommand::ConstPtr& cmd) {
...
kr_mav_msgs::PositionCommand::ConstPtr
LineTrackerYaw::update(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  if (goal_reached_) {
    cmd->position.x = goal_pos_(0), cmd->position.y = goal_pos_(1),
    cmd->position.z = goal_pos_(2);
  ...
  if (traj_time <= t_yaw_accel_) {
    ...
    yaw = start_yaw_ +
          (0.5f * yaw_a_des_ * yaw_dir_ * t_yaw_accel_ * t_yaw_accel_) +
          yaw_dot * dT_yaw;
  } else if (traj_time <= (t_yaw_accel_ + t_yaw_constant_ + t_yaw_accel_)) {
    // Decelerate
    const float dT_yaw = traj_time - (t_yaw_accel_ + t_yaw_constant_);
    yaw_dot =
        yaw_a_des_ * yaw_dir_ * t_yaw_accel_ - yaw_a_des_ * yaw_dir_ * dT_yaw;
    yaw = start_yaw_ +
          (0.5f * yaw_a_des_ * yaw_dir_ * t_yaw_accel_ * t_yaw_accel_) +
          (yaw_a_des_ * yaw_dir_ * t_yaw_accel_ * t_yaw_constant_) +
          (yaw_a_des_ * yaw_dir_ * t_yaw_accel_ * dT_yaw -
           0.5f * yaw_a_des_ * yaw_dir_ * dT_yaw * dT_yaw);
  } else {
    // Reached goal
  ...
  } else if (d > translation_dist_) // Overshoot
  {
    ...
    v = Eigen::Vector3f::Zero();
    x = goal_pos_;
  } else if (d >= (translation_dist_ - ramp_dist) &&
             d <= translation_dist_) // Decelerate
  {
    a = -a_des_ * translation_dir_;
    v = std::sqrt(2 * a_des_ * (translation_dist_ - d)) * translation_dir_;
    x = proj + v * dT + 0.5f * a * dT * dT;
  } else if (d > ramp_dist &&
             d < translation_dist_ - ramp_dist) // Constant velocity
  {
    a = Eigen::Vector3f::Zero();
...
void LineTrackerYaw::goal_callback(
    const kr_tracker_msgs::LineTrackerGoal::ConstPtr& msg) {
  goal_pos_(0) = msg->x;
  goal_pos_(1) = msg->y;
  goal_pos_(2) = msg->z;
  goal_yaw_ = msg->yaw;
  ...
  // Compute times for accel and constant vel stages of trapezoidal yaw velocity
  // profile
