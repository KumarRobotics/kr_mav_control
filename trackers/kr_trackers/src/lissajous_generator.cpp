...
void LissajousGenerator::setParams(
    const kr_tracker_msgs::LissajousTrackerGoal::ConstPtr& msg) {
  x_amp_ = msg->x_amp;
  y_amp_ = msg->y_amp;
  z_amp_ = msg->z_amp;
  ...
  ramp_s_ =
      a7_ * tr8 / 8.0 + a6_ * tr7 / 7.0 + a5_ * tr6 / 6.0 + a4_ * tr5 / 5.0;
...
void LissajousGenerator::setParams(
    const kr_tracker_msgs::LissajousAdderGoal::ConstPtr& msg, int num) {
...
const kr_mav_msgs::PositionCommand::Ptr
LissajousGenerator::getPositionCmd(void) {
  if (!active_) {
  ...
  if (t > total_time_) {
    ...
    yaw = 0;
    cmd->position.x = pos(0), cmd->position.y = pos(1),
    ...
    if (t < ramp_time_) {
      ...
      sdddot =
          42.0 * a7_ * t5 + 30.0 * a6_ * t4 + 20.0 * a5_ * t3 + 12.0 * a4_ * t2;
      ...
      s = 2.0 * ramp_s_ + const_time_ - a7_ * te8 / 8.0 - a6_ * te7 / 7.0 -
          a5_ * te6 / 6.0 - a4_ * te5 / 5.0;
      sdot = a7_ * te7 + a6_ * te6 + a5_ * te5 + a4_ * te4;
      sddot = -7.0 * a7_ * te6 - 6.0 * a6_ * te5 - 5.0 * a5_ * te4 -
              4.0 * a4_ * te3;
      sdddot = 42.0 * a7_ * te5 + 30.0 * a6_ * te4 + 20.0 * a5_ * te3 +
               12.0 * a4_ * te2;
    }
    double T = period_;
    double T2 = T * T;
    ...
    vel(0) = x_amp_ * 2 * M_PI * x_num_periods_ *
             std::sin(2 * M_PI * x_num_periods_ * s / T) * sdot / T;
    vel(1) = y_amp_ * 2 * M_PI * y_num_periods_ *
             std::cos(2 * M_PI * y_num_periods_ * s / T) * sdot / T;
    vel(2) = z_amp_ * 2 * M_PI * z_num_periods_ *
             std::cos(2 * M_PI * z_num_periods_ * s / T) * sdot / T;
    acc(0) =
        x_amp_ *
        (4 * M_PI * M_PI * x_num_periods_ * x_num_periods_ *
             std::cos(2 * M_PI * x_num_periods_ * s / T) * sdot * sdot / T2 +
         2 * M_PI * x_num_periods_ *
             std::sin(2 * M_PI * x_num_periods_ * s / T) * sddot / T);
    acc(1) =
        y_amp_ *
        (-4 * M_PI * M_PI * y_num_periods_ * y_num_periods_ *
             std::sin(2 * M_PI * y_num_periods_ * s / T) * sdot * sdot / T2 +
         2 * M_PI * y_num_periods_ *
             std::cos(2 * M_PI * y_num_periods_ * s / T) * sddot / T);
    acc(2) =
        z_amp_ *
        (-4 * M_PI * M_PI * z_num_periods_ * z_num_periods_ *
             std::sin(2 * M_PI * z_num_periods_ * s / T) * sdot * sdot / T2 +
         2 * M_PI * z_num_periods_ *
             std::cos(2 * M_PI * z_num_periods_ * s / T) * sddot / T);
    jrk(0) =
        x_amp_ *
        (-8 * M_PI * M_PI * M_PI * x_num_periods_ * x_num_periods_ *
             x_num_periods_ * std::sin(2 * M_PI * x_num_periods_ * s / T) *
             sdot * sdot * sdot / T3 +
         4 * M_PI * M_PI * x_num_periods_ * x_num_periods_ *
             std::cos(2 * M_PI * x_num_periods_ * s / T) * sdot * sddot / T2 +
         2 * M_PI * x_num_periods_ *
             std::sin(2 * M_PI * x_num_periods_ * s / T) * sdddot / T);
    jrk(1) =
        y_amp_ *
        (-8 * M_PI * M_PI * M_PI * y_num_periods_ * y_num_periods_ *
             y_num_periods_ * std::cos(2 * M_PI * y_num_periods_ * s / T) *
             sdot * sdot * sdot / T3 -
         4 * M_PI * M_PI * y_num_periods_ * y_num_periods_ *
             std::sin(2 * M_PI * y_num_periods_ * s / T) * sdot * sddot / T2 +
         2 * M_PI * y_num_periods_ *
             std::cos(2 * M_PI * y_num_periods_ * s / T) * sdddot / T);
    jrk(2) =
        z_amp_ *
        (-8 * M_PI * M_PI * M_PI * z_num_periods_ * z_num_periods_ *
             z_num_periods_ * std::cos(2 * M_PI * z_num_periods_ * s / T) *
             sdot * sdot * sdot / T3 -
         4 * M_PI * M_PI * z_num_periods_ * z_num_periods_ *
             std::sin(2 * M_PI * z_num_periods_ * s / T) * sdot * sddot / T2 +
         2 * M_PI * z_num_periods_ *
             std::cos(2 * M_PI * z_num_periods_ * s / T) * sdddot / T);
    yaw = yaw_amp_ * (1 - std::cos(2 * M_PI * yaw_num_periods_ * s / T));
    yaw_dot = yaw_amp_ * 2 * M_PI * yaw_num_periods_ *
              std::sin(2 * M_PI * yaw_num_periods_ * s / T) * sdot / T;
    cmd->position.x = pos(0), cmd->position.y = pos(1),
    cmd->position.z = pos(2);
    cmd->velocity.x = vel(0), cmd->velocity.y = vel(1),
...
void LissajousGenerator::generatePath(nav_msgs::Path& path,
  ...
  if (goal_set_) {
    ...
    while (s < period_) {
      ...
      ps.pose.position.x =
          x_amp_ * (1 - std::cos(2 * M_PI * x_num_periods_ * s / T)) +
          initial_pt.x;
      ps.pose.position.y =
          y_amp_ * std::sin(2 * M_PI * y_num_periods_ * s / T) + initial_pt.y;
      ps.pose.position.z =
          z_amp_ * std::sin(2 * M_PI * z_num_periods_ * s / T) + initial_pt.z;
...
bool LissajousGenerator::isActive(void) { return active_; }

bool LissajousGenerator::goalIsSet(void) { return goal_set_; }

bool LissajousGenerator::status() const {
  return goal_reached_ ? kr_tracker_msgs::TrackerStatus::SUCCEEDED
