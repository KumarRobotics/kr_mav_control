...
private:
  ...
  // Must be a pointer because plugin does not support a constructor with
  // inputs, but an action server must be initialized with a Nodehandle.
...
void TrajectoryTracker::Initialize(const ros::NodeHandle& nh) {
  ...
  priv_nh.param(
      "derivative_order_to_minimize", derivative_order_to_minimize, 3);
  ...
  traj_gen_.reset(new TrajectoryGenerator(continuous_derivative_order,
                                          derivative_order_to_minimize));
  ...
  tracker_server_->registerGoalCallback(
      boost::bind(&TrajectoryTracker::goal_callback, this));
  tracker_server_->registerPreemptCallback(
      boost::bind(&TrajectoryTracker::preempt_callback, this));
...
bool TrajectoryTracker::Activate(
  ...
  if (goal_set_ && pos_set_) {
    if (!tracker_server_->isActive()) {
      ROS_WARN("TrajectoryTracker::Activate: goal_set_ is true but action "
...
void TrajectoryTracker::Deactivate(void) {
  if (tracker_server_->isActive()) {
    ROS_WARN("TrajectoryTracker::Deactivate: deactivated tracker while still "
             "tracking the goal.");
...
kr_mav_msgs::PositionCommand::ConstPtr
TrajectoryTracker::update(const nav_msgs::Odometry::ConstPtr& msg) {
  pos_set_ = true;
  ...
  const float dx = Eigen::Vector3f(current_pos_(0) - msg->pose.pose.position.x,
  ...
  if (goal_set_) {
    ...
    for (const auto& p : goal_.waypoints) {
      traj_gen_->addWaypoint(
          Eigen::Vector3f(p.position.x, p.position.y, p.position.z));
    }
    std::vector<float> waypoint_times;
    waypoint_times.reserve(goal_.waypoints.size());
    if (goal_.waypoint_times.size() == 0) {
      waypoint_times =
          traj_gen_->computeTimesTrapezoidSpeed(max_v_des_ / 2, max_a_des_ / 2);
    } else {
  ...
  if (traj_time >= traj_total_time_) // Reached goal
  {
    ...
    x = Eigen::Vector3f(
        last_waypoint_pos.x, last_waypoint_pos.y, last_waypoint_pos.z);
    yaw_des = ICs_.yaw();
    yaw_dot_des = 0;
    goal_reached_ = true;
  } else if (traj_time >= 0) {
...
void TrajectoryTracker::goal_callback() {
  // If another goal is already active, cancel that goal and track this one
  // instead.
  if (tracker_server_->isActive()) {
  ...
  // If preempt has been requested, then set this goal to preempted and make no
  // changes to the tracker state.
  ...
  if (msg->waypoints.size() > 0 &&
      (msg->waypoint_times.size() == 0 ||
       msg->waypoint_times.size() == msg->waypoints.size())) {
    goal_ = *msg;
    goal_set_ = true;
    goal_reached_ = false;
  } else {
