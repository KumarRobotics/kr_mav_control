...
private:
  ...
  typedef actionlib::SimpleActionServer<kr_tracker_msgs::LissajousTrackerAction>
      ServerType;
...
void LissajousTracker::Initialize(const ros::NodeHandle& nh) {
  ...
  tracker_server_ = std::shared_ptr<ServerType>(
      new ServerType(priv_nh, "LissajousTracker", false));
  tracker_server_->registerGoalCallback(
      boost::bind(&LissajousTracker::goal_callback, this));
  tracker_server_->registerPreemptCallback(
      boost::bind(&LissajousTracker::preempt_callback, this));
  tracker_server_->start();
}

bool LissajousTracker::Activate(
    const kr_mav_msgs::PositionCommand::ConstPtr& cmd) {
  // Only allow activation if a goal has been set
  if (generator_.goalIsSet()) {
    if (!tracker_server_->isActive()) {
      ROS_WARN("LissajousTracker::Activate: goal_set is true but action server "
...
kr_mav_msgs::PositionCommand::ConstPtr
LissajousTracker::update(const nav_msgs::Odometry::ConstPtr& msg) {
...
void LissajousTracker::goal_callback(void) {
  ...
  kr_tracker_msgs::LissajousTrackerGoal::ConstPtr msg =
