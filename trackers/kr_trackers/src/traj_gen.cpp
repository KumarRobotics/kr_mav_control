...
void TrajectoryGenerator::setInitialConditions(const Vec3f& pos,
  ...
  for (size_t i = 0;
       i < std::min(initial_derivatives_.size(), derivatives.size());
       ++i) {
...
std::vector<float>
TrajectoryGenerator::computeTimesTrapezoidSpeed(float v_des,
                                                float a_des) const {
...
std::vector<float>
TrajectoryGenerator::computeTimesConstantSpeed(float avg_speed) const {
  std::vector<float> waypoint_times;
  waypoint_times.reserve(waypoints_.size());
...
template <typename T> T powInt(T x, unsigned int n) {
  if (n == 0)
    return T{1};
...
bool TrajectoryGenerator::calculate(const std::vector<float>& waypoint_times) {
  ...
  M.block(
      num_segments * N_ - N_ / 2, N_ / 2 + num_waypoints - 2, N_ / 2, N_ / 2) =
  ...
  std::cout << "R:\n" << R << std::endl;
#endif
  Eigen::MatrixXf Rpp = R.block(num_fixed_derivatives,
                                num_fixed_derivatives,
                                num_free_derivatives,
                                num_free_derivatives);
  Eigen::MatrixXf Rpf = R.block(
      num_fixed_derivatives, 0, num_free_derivatives, num_fixed_derivatives);
...
bool TrajectoryGenerator::getCommand(
    const float time, Vec3f& pos, Vec3f& vel, Vec3f& acc, Vec3f& jrk) const {
  if (time < 0)
    return false;
  ...
  for (unsigned int i = 3; i < p.rows(); i++)
    jrk +=
        p.row(i).transpose() * (i * (i - 1) * (i - 2) * powInt(t_traj, i - 3));
...
void TrajectoryGenerator::calcMaxPerSegment(std::vector<float>& max_vel,
  ...
  for (unsigned int seg_idx = 0; seg_idx < waypoint_times_.size() - 1;
       ++seg_idx) {
