...
void LineTrackerDistance::Initialize(const ros::NodeHandle& nh) {
  ...
  tracker_server_->registerGoalCallback(
      boost::bind(&LineTrackerDistance::goal_callback, this));
  tracker_server_->registerPreemptCallback(
      boost::bind(&LineTrackerDistance::preempt_callback, this));

  tracker_server_->start();
}

bool LineTrackerDistance::Activate(
  ...
  if (goal_set_ && pos_set_) {
    ...
    if (!tracker_server_->isActive()) {
      ROS_WARN("LineTrackerDistance::Activate: goal_set_ is true but action "
...
void LineTrackerDistance::Deactivate(void) {
  if (tracker_server_->isActive()) {
    ROS_WARN("LineTrackerDistance::Deactivate: deactivated tracker while still "
             "tracking the goal.");
...
kr_mav_msgs::PositionCommand::ConstPtr
LineTrackerDistance::update(const nav_msgs::Odometry::ConstPtr& msg) {
  // Record distance between last position and current.
  const float dx = Eigen::Vector3f((pos_(0) - msg->pose.pose.position.x),
  ...
  Eigen::Vector3f x(pos_), v(Eigen::Vector3f::Zero()),
...
void LineTrackerDistance::goal_callback() {
  ...
  if (tracker_server_->isActive()) {
    ROS_INFO("LineTrackerDistance goal (%2.2f, %2.2f, %2.2f) aborted.",
             goal_(0),
             goal_(1),
             goal_(2));
    tracker_server_->setAborted();
  ...
  if (tracker_server_->isPreemptRequested()) {
    ROS_INFO(
        "LineTrackerDistance going to goal (%2.2f, %2.2f, %2.2f) preempted.",
        msg->x,
        msg->y,
        msg->z);
...
void LineTrackerDistance::preempt_callback() {
  if (tracker_server_->isActive()) {
    ROS_INFO("LineTrackerDistance going to goal (%2.2f, %2.2f, %2.2f) aborted.",
             goal_(0),
             goal_(1),
             goal_(2));
    tracker_server_->setAborted();
