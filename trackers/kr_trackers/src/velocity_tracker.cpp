...
VelocityTracker::VelocityTracker(void)
    : odom_set_(false), active_(false), use_position_gains_(false), last_t_(0) {
}
...
void VelocityTracker::Initialize(const ros::NodeHandle& nh) {
  ...
  sub_vel_cmd_ = priv_nh.subscribe("goal",
                                   10,
                                   &VelocityTracker::velocity_cmd_cb,
                                   this,
                                   ros::TransportHints().tcpNoDelay());
}

bool VelocityTracker::Activate(
...
kr_mav_msgs::PositionCommand::ConstPtr
VelocityTracker::update(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  if ((ros::Time::now() - last_cmd_time_).toSec() > timeout_) {
    ...
    if (use_position_gains_)
      position_cmd_.use_msg_gains_flags =
          kr_mav_msgs::PositionCommand::USE_MSG_GAINS_NONE;
    ...
    return kr_mav_msgs::PositionCommand::ConstPtr(
        new kr_mav_msgs::PositionCommand(position_cmd_));
  }
  ...
  if (use_position_gains_) {
    ...
    position_cmd_.position.x =
        position_cmd_.position.x + dt * position_cmd_.velocity.x;
    position_cmd_.position.y =
        position_cmd_.position.y + dt * position_cmd_.velocity.y;
    position_cmd_.position.z =
        position_cmd_.position.z + dt * position_cmd_.velocity.z;
  } else {
    position_cmd_.kx[0] = 0, position_cmd_.kx[1] = 0, position_cmd_.kx[2] = 0;
    position_cmd_.use_msg_gains_flags =
        kr_mav_msgs::PositionCommand::USE_MSG_GAINS_POSITION_ALL;
...
void VelocityTracker::velocity_cmd_cb(
    const kr_tracker_msgs::VelocityGoal::ConstPtr& msg) {
  // ROS_INFO("VelocityTracker goal (%2.2f, %2.2f, %2.2f, %2.2f)", msg->vx,
  // msg->vy, msg->vz, msg->vyaw);
