...
void LineTrackerTrapezoid::Initialize(const ros::NodeHandle& nh) {
  ...
  sub_goal_ = priv_nh.subscribe("goal",
                                10,
                                &LineTrackerTrapezoid::goal_callback,
                                this,
                                ros::TransportHints().tcpNoDelay());
}

bool LineTrackerTrapezoid::Activate(
...
kr_mav_msgs::PositionCommand::ConstPtr
LineTrackerTrapezoid::update(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  if (traj_time <= t_accel_) {
    ...
    x = (start_pos_ + 0.5 * a_des_ * dir * t_accel_ * t_accel_) +
        (a_des_ * dir * t_accel_ * t_constant_) +
...
void LineTrackerTrapezoid::goal_callback(
    const kr_mav_msgs::LineTrackerGoal::ConstPtr& msg) {
  goal_(0) = msg->x;
  goal_(1) = msg->y;
  goal_(2) = msg->z;
